...
local Tinfo = TweenInfo.new(7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
...
script.Parent.Open.Event:Connect(function()
	open.Value = true
	Moving = true
	script.Parent.GlobalAlertSound:Play()
	...
	wait(8)
	Moving = false
	script.Parent.GlobalAlertSound:Stop()
end)
...
script.Parent.OpenTrigger.Touched:Connect(function()
	script.Parent.Open:Fire()
	script.Parent.OpenTrigger.CanTouch = false
end)

script.Parent.CloseTrigger.Touched:Connect(function()
	script.Parent.CloseTrigger.CanTouch = false
	script.Parent.Barrier.CanCollide = true
	repeat wait()
	until Moving == false
	script.Parent.Close:Fire()
end)